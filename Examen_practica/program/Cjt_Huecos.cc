...
bool Cjt_Huecos::esHueco(const Ubicacion U) {
    set<Segmento>::iterator it = Cjt_H.begin();
    //Si no hi han huecos, llavors no pot ser un hueco
    if (it == Cjt_H.end()) return false;
    //Si és un lloc invàlid, tampoc pot ser un hueco
    if (U.hilera() < 0 or U.plaza() < 0 or U.piso() < 0) return false;
    else {
        //Busca un hueco
        for (it = Cjt_H.begin(); it != Cjt_H.end(); ++it) {
            //Si U està dins del hueco, llavors es considera hueco
            if (U.hilera() == (*it).ubic().hilera() and U.piso() == (*it).ubic().piso()) {
                if (U.plaza() >= (*it).ubic().plaza() and U.plaza() < (*it).ubic().plaza() + (*it).longitud())
                return true;
            }
        }
    }
